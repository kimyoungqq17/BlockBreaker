...
public class Block implements Serializable {
	...
	public Block(int aRed, int aGreen, int aBlue, int aPoints, Game aGame) {
		...
		/*
		 * While we aren't supposed to model model code, there was an issue being
		 * cause by ID's being counted up from 1 every time the application was closed
		 * and reopened (since ID was not being saved). This caused issues when trying
		 * to create new blocks with old blocks existing with equal or higher ID. This
		 * solution will make sure ID's stay unique, fixing all fetching issues.
		 */
		boolean uniqueID = true, added;
		do {
			added = false;
			for (Block gameBlock : Block223Application.getCurrentGame().getBlocks())
				if (gameBlock.getId() == id) {
					id = nextId++;
					uniqueID = false;
					added = true;
					break;
	...
	public BlockAssignment addBlockAssignment(int aGridHorizontalPosition, int aGridVerticalPosition, Level aLevel,
			Game aGame) {
	...
	public boolean addBlockAssignmentAt(BlockAssignment aBlockAssignment, int index) {
		...
		if (addBlockAssignment(aBlockAssignment)) {
			...
			if (index > numberOfBlockAssignments()) {
				index = numberOfBlockAssignments() - 1;
			}
			blockAssignments.remove(aBlockAssignment);
	...
	public String toString() {
		return super.toString() + "[" + "id" + ":" + getId() + "," + "red" + ":" + getRed() + "," + "green" + ":"
				+ getGreen() + "," + "blue" + ":" + getBlue() + "," + "points" + ":" + getPoints() + "]"
